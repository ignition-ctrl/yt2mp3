...
def download_music(ytname=None, filename=None, heredeterminer=None):
    ...
    if ytname == None: #so, here it is. If ytname is None, interactively have the user put a link in, and if it's not valid exit out.
        ...
        if re.match("http(?:s?):\/\/(?:www\.)?youtu(?:be\.com\/watch\?v=|\.be\/)([\w\-\_]*)(&(amp;)?‌​[\w\?‌​=]*)?", linkfinder):
            #IGNOREdownload_music(str(linkfinder), filename, heredeterminer) #This sends the user through the function again, with linkfinder taking ytname
            """
            ...
            #IGNOREsince linkfinder is not None, it won't trigger this code again.
            #IGNOREprint("exit")
            #IGNOREexit(0) #However, once download_music finished, it will continue at the first iteration. The exit guarantees there's no loop
        else:
    ...
    if filename == None:
        ...
        #IGNOREdownload_music(ytname, str(namefinder), heredeterminer)
        filename = str(namefinder)
        #IGNOREprint("exit")
        #IGNOREexit(0) # same sort of None verifying. It's really genius, whoever put this in python
    if heredeterminer == None:
        herefinder = input("Do you want to download the file in the current directory?\n")
        #IGNOREdownload_music(ytname, filename, str(herefinder)) #Now, at this point, this should be the final iteration of the function, since all arguments are supplied
        heredeterminer = str(herefinder)
        #IGNOREprint("exit")
        #IGNOREexit(0) #should be the final exit out of the iterations
    ...
    params = {
            ...
            #updated, put the paths in seperate variables with ternary operators of their own, for optional default names.
}
    ...
    audiofile.tag.save()
    #Some simple querying in order to put the right tags. 
